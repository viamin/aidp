...
RSpec.describe "Harness Backward Compatibility", type: :compatibility do
  let(:project_dir) { Dir.mktmpdir("aidp_harness_compatibility_test") }
  ...
  describe "CLI Command Backward Compatibility" do
    ...
    it "step-specific analyze commands work exactly as before" do
      ...
      expect(result[:status]).to eq("completed")
      expect(result[:output]).to be_nil # Output may be nil in mock mode
    end
    ...
    it "jobs command works exactly as before" do
      # Test that jobs command works
      expect { cli.jobs }.not_to raise_error
    end
  end
  ...
  describe "Progress Tracking Backward Compatibility" do
    ...
    it "harness progress extends existing progress" do
      # Test that harness progress extends existing progress
      progress_data = {
      ...
      # Test that both existing and harness progress are loaded
      progress = Aidp::Analyze::Progress.new(project_dir)
  ...
  describe "Provider Integration Backward Compatibility" do
    ...
    it "harness providers extend existing providers" do
      # Test that harness providers extend existing providers
      expect(Aidp::Providers::SupervisedBase).to be_a(Class)
      expect(Aidp::Providers::SupervisedCursor).to be_a(Class)
    end
  end
  ...
  describe "Error Handling Backward Compatibility" do
    ...
    it "harness error handling extends existing error handling" do
      # Test that harness error handling extends existing error handling
      expect(Aidp::Harness::ErrorHandler).to be_a(Class)
    end
  end
